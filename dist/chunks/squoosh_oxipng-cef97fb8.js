...
waitForMsgType(self, 'wasm_bindgen_worker_init').then(async data => {
  ...
  // the main thread).
  const pkg = await Promise.resolve().then(function () { return squoosh_oxipng; });
  ...
  try {
    await Promise.all(
      Array.from({ length: builder.numThreads() }, () => {
        ...
        const worker = new Worker(new URL("workerHelpers-452e23c5.js", import.meta.url), {
          type: 'module'
        });
...
async function init(input, maybe_memory) {
    if (typeof input === 'undefined') {
        input = new URL("../assets/squoosh_oxipng_bg-5c8fadb7.wasm", import.meta.url);
    }
    const imports = {};
    imports.wbg = {};
